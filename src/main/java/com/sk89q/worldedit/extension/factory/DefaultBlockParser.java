...
class DefaultBlockParser extends InputParser<BaseBlock> {
    ...
    public BaseBlock parseFromInput(String input, ParserContext context) throws InputParseException {
        // TODO: Rewrite this entire method to use BaseBlocks and ignore BlockType, as well as to properly handle
        // mod:name IDs
        ...
        if ("hand".equalsIgnoreCase(testId)) {
            ...
            try {
                primaryPosition = context.requireSession()
            ...
            if (!context.requireWorld()
                .isValidBlockType(blockId)) {
            ...
            case MOB_SPAWNER:
                ...
                if (blockAndExtraData.length > 1) {
                    ...
                    for (MobType mobType : MobType.values()) {
                        if (mobType.getName()
                            .toLowerCase()
                            .equals(mobName.toLowerCase())) {
                            mobName = mobType.getName();
                            break;
                        }
                    }
                    if (!worldEdit.getServer()
                        .isValidMobType(mobName)) {
